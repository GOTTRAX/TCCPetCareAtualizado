...
// Inicia sessão no topo (sem espaços antes do <?php)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($acao === 'servico_atualizar') {
            $sql = "UPDATE Servicos 
                    SET nome=:n, descricao=:d, preco_normal=:pn, preco_feriado=:pf 
                    WHERE id=:id";
            $st = $pdo->prepare($sql);
            $st->execute([
                ':n'=>trim($_POST['nome']??''),
        ...
        if ($acao === 'dias_salvar') {
            ...
            foreach ($dias as $k => $dado) {
                ...
                if ($row) {
                    ...
                    $ins = $pdo->prepare("INSERT INTO Dias_Trabalhados 
                                          (dia_semana, horario_abertura, horario_fechamento, ativo) 
                                          VALUES (:ds,:a,:f,:atv)");
                    $ins->execute([
...
// Só inclui o header se não for POST (para evitar enviar HTML antes do redirect)
include("header.php");
