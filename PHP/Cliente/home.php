...
<script>
    document.addEventListener('auxclick', function(event) {
        // Verifica se o botão clicado é o botão 4 (índice 3)
        if (event.button === 3) {
            // Impede o comportamento padrão do navegador (ex.: voltar)
            event.preventDefault();

            // Chama o script de logout para encerrar a sessão
            fetch('http://localhost:8080/bruno/TCCPetCareAtualizado/PHP/Cliente/logout.php', {
                method: 'POST', // Método POST para garantir segurança
                credentials: 'same-origin' // Inclui cookies, se necessário
            })
            .then(response => {
                // Redireciona para a página de login após o logout
                window.location.href = 'http://localhost:8080/bruno/TCCPetCareAtualizado/PHP/Cliente/login.php';
            })
            .catch(error => {
                console.error('Erro ao fazer logout:', error);
                // Redireciona mesmo em caso de erro, para garantir o fluxo
                window.location.href = 'http://localhost:8080/bruno/TCCPetCareAtualizado/PHP/Cliente/login.php';
            });
        }
    });
